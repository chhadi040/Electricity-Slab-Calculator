    ...
    void editCustomer() {
        ...
        while (current != nullptr) {
            if (current->id == searchId) {
                ...
                cout << "Enter new units (leave empty to keep current): ";
                getline(cin, input);
                if (!input.empty()) {
                    try {
                        int newUnits = stoi(input);
                        if (newUnits < 0) {
                            cout << "Units cannot be negative. Keeping previous value.\n";
                        } else {
                            current->units = newUnits;
                        }
                    } catch (...) {
                        cout << "Invalid units input. Keeping previous value.\n";
                    }
                }

                current->billAmount = calculateBill(current->units);
                cout << "\nRecord updated successfully.\n";
                cout << "Updated Bill Amount: Rs. " << fixed << setprecision(2)
                     << current->billAmount << endl;
                return;
            }
